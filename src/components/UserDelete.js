...
export default function UserDelete(props) {
    ...
    const deleteUser = async dataForm => {
        try {
            const response = await deleteUserApi(props.userSelected._id,getToken());
            toast.success("Le livre "+ props.userSelected.userName +" a été supprimé")
            props.deleteUser(props.userSelected._id)
            setIsOpen(false);
    ...
    return(
        <div className="UserInfo">
            <button id="deleteUser" className="btn w-100 btn-block btn-danger" onClick={openModal}>
            ...
            <Modal isOpen={modalIsOpen} onRequestClose={closeModal} style={customStyles} contentLabel="Example Modal" shouldCloseOnOverlayClick={false}>
               Êtes-vous sûr de vouloir supprimer l'utilisateur <b>{props.userSelected.userName}</b> ?
               <br/><br/>
                <div className="row">
                    <div className="col">
                    ...
                    <div className="col">
                        <button className="btn btn-success btn-block w-100" type="submit" onClick={deleteUser}>
